      ...
      <div className="screen" style={{ paddingTop: 8 }}>
              ...
              return (
                <div key={m.key} className="row">
                  ...
                  <span className="row-main">
                    <span className="row-title">{m.label} {count(m.key as keyof WriteSelection) && <span className="caption">· {count(m.key as keyof WriteSelection)}</span>}</span>
        ...
        {CAN_WRITE_TAG ? (
          <button className="btn btn-primary" disabled={over || !anySelected} onClick={doWrite}>
            <Icon.nfc size={20} /> Write {size} bytes to tag
          </button>
        ) : (
          <>
